...
from custom_components.phoenix_mcp.version_store import VersionRecord, VersionStore
...
@pytest.mark.asyncio
async def test_list_recent_offset_pages_through_the_full_feed():
    store = VersionStore()
    await _rec(store, rt="automation", rid="a", after={"n": 1})
    await _rec(store, rt="script", rid="b", after={"n": 2})
    await _rec(store, rt="scene", rid="c", after={"n": 3})
    page1 = store.list_recent(2, 0)
    page2 = store.list_recent(2, 2)
    assert [r.resource_id for r in page1] == ["c", "b"]
    assert [r.resource_id for r in page2] == ["a"]


@pytest.mark.asyncio
async def test_count_recent_matches_total_regardless_of_paging():
    store = VersionStore()
    for i in range(5):
        await _rec(store, rt="automation", rid=str(i), after={"n": i})
    assert store.count_recent() == 5
    assert len(store.list_recent(2, 0)) == 2
    assert store.count_recent() == 5
