...
from custom_components.phoenix_mcp.rate_limiter import (
    WINDOW_SECONDS,
    RateLimitResult,
    RateLimiter,
)
...
class TestAllowedRequests:
    ...
    def test_reset_based_on_oldest_entry(self):
        t0 = 1_000_000.0
        rl = _rl()
        with patch("custom_components.phoenix_mcp.rate_limiter.time.time", return_value=t0), \
             patch("custom_components.phoenix_mcp.rate_limiter.time.monotonic", return_value=t0):
            _check(rl, requests=5, burst=10)
        with patch("custom_components.phoenix_mcp.rate_limiter.time.time", return_value=t0 + 10), \
             patch("custom_components.phoenix_mcp.rate_limiter.time.monotonic", return_value=t0 + 10):
            result = _check(rl, requests=5, burst=10)
...
class TestSlidingWindow:
    ...
    def test_window_expiry_allows_new_requests(self):
        ...
        with patch("custom_components.phoenix_mcp.rate_limiter.time.time", return_value=t0 + 61), \
             patch("custom_components.phoenix_mcp.rate_limiter.time.monotonic", return_value=t0 + 61):
            result = _check(rl, requests=5, burst=10)
            assert result.allowed is True
    ...
    def test_partial_window_expiry(self):
        ...
        with patch("custom_components.phoenix_mcp.rate_limiter.time.time", return_value=t0 + 30), \
             patch("custom_components.phoenix_mcp.rate_limiter.time.monotonic", return_value=t0 + 30):
            for _ in range(2):
                _check(rl, requests=5, burst=10)
    ...
    def test_retry_after_minimum_is_one(self):
        ...
        with patch("custom_components.phoenix_mcp.rate_limiter.time.time", return_value=t0 + 59.999), \
             patch("custom_components.phoenix_mcp.rate_limiter.time.monotonic", return_value=t0 + 59.999):
            result = _check(rl, requests=5, burst=10)
...
class TestBurstLimit:
    ...
    def test_burst_window_expiry_allows_new_requests(self):
        ...
        with patch("custom_components.phoenix_mcp.rate_limiter.time.time", return_value=t0 + 1.001), \
             patch("custom_components.phoenix_mcp.rate_limiter.time.monotonic", return_value=t0 + 1.001):
            result = rl.check("tok", rate_limit_requests=100, rate_limit_burst=3)
            assert result.allowed is True
    ...
    def test_burst_allowed_after_some_entries_age_out(self):
        ...
        with patch("custom_components.phoenix_mcp.rate_limiter.time.time", return_value=t0), \
            ...
            for _ in range(2):
                rl.check("tok", rate_limit_requests=100, rate_limit_burst=3)
        with patch("custom_components.phoenix_mcp.rate_limiter.time.time", return_value=t0 + 0.5), \
             patch("custom_components.phoenix_mcp.rate_limiter.time.monotonic", return_value=t0 + 0.5):
            rl.check("tok", rate_limit_requests=100, rate_limit_burst=3)
            denied = rl.check("tok", rate_limit_requests=100, rate_limit_burst=3)
            assert denied.allowed is False
        # Advance 1.1 s from t0; the two entries at t0 have aged out of burst window
        with patch("custom_components.phoenix_mcp.rate_limiter.time.time", return_value=t0 + 1.1), \
             patch("custom_components.phoenix_mcp.rate_limiter.time.monotonic", return_value=t0 + 1.1):
            result = rl.check("tok", rate_limit_requests=100, rate_limit_burst=3)
            assert result.allowed is True
    ...
    def test_sliding_window_still_enforced_when_burst_passes(self):
        ...
        for i in range(5):
            with patch("custom_components.phoenix_mcp.rate_limiter.time.time", return_value=t0 + i * 5), \
                 patch("custom_components.phoenix_mcp.rate_limiter.time.monotonic", return_value=t0 + i * 5):
        ...
        with patch("custom_components.phoenix_mcp.rate_limiter.time.time", return_value=t0 + 25), \
             patch("custom_components.phoenix_mcp.rate_limiter.time.monotonic", return_value=t0 + 25):
