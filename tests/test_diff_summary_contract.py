...
def test_every_template_is_reachable() -> None:
    ...
    f"hass_turn.{verb}", f"dashboard_card.{op}") have no literal to match, so
    they are covered by prefix. That list doubles as the record of which keys
    are computed rather than written out.
    """
    computed_prefixes = ("blueprint.", "dashboard.", "dashboard_card.", "hass_turn.")
