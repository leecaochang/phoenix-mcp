...
from custom_components.phoenix_mcp.personas import (
    PERSONA_DEFINITIONS,
    detect_persona,
    get_persona_caps,
    matches_persona,
...
class TestEsphome:
    """The esphome persona is a narrow, one-click template for device work."""

    def test_only_the_esphome_cap_is_writable(self):
        caps = get_persona_caps(PERSONA_ESPHOME)
        writable = {c for c, m in caps.items() if m != CAP_DENY and c.endswith("_write")}
        assert writable == set()
        assert caps["cap_esphome_yaml"] == CAP_CONFIRM
        # No device control and no HA config authoring: this token edits device
        # files, it does not operate the house.
        assert caps["cap_physical_control"] == CAP_DENY
        assert caps["cap_yaml_edit"] == CAP_DENY
        assert caps["cap_filesystem"] == CAP_DENY
        assert caps["cap_restart"] == CAP_DENY

    def test_diagnostic_reads_allowed(self):
        caps = get_persona_caps(PERSONA_ESPHOME)
        for cap in ("cap_config_read", "cap_search", "cap_registry_read",
                    "cap_diagnostics", "cap_log_read"):
            assert caps[cap] == CAP_ALLOW, cap

    def test_detected_from_its_own_caps(self):
        assert detect_persona(get_persona_caps(PERSONA_ESPHOME)) == PERSONA_ESPHOME


class TestEsphomeCapDefaults:
    def test_no_other_persona_grants_esphome_yaml(self):
        """Only home_admin (confirm) and esphome (confirm) may touch device YAML.

        A new cap must not silently widen an existing persona; every other preset
        stays at deny so applying a familiar persona never grants device-file access.
        """
        granted = {
            name for name, caps in PERSONA_DEFINITIONS.items()
            if caps["cap_esphome_yaml"] != CAP_DENY
        }
        assert granted == {PERSONA_HOME_ADMIN, PERSONA_ESPHOME}
