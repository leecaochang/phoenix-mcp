...
from custom_components.phoenix_mcp.helpers import build_safe_config
...
_RAW = {
    ...
    "version": "2026.6.0",
    "components": ["light", "phoenix_mcp", "phoenix_mcp.sensor", "automation"],
    "currency": "USD",
    "country": "US",
    "language": "en",
...
def test_strips_phoenix_components():
    safe = build_safe_config(_hass_with_config(_RAW))
    assert "phoenix_mcp" not in safe["components"]
    assert "phoenix_mcp.sensor" not in safe["components"]
