...
interface Props {
  ...
  defaultOpen?: boolean;
  persistKey?: string;
  // Path (relative to the docs site) for this section's "?" documentation
  // link. Rendered outside the toggle button: nesting a link inside a
  // button is invalid HTML and would fire the toggle on click.
  docsPath?: string;
...
export function CollapsibleCard({ title, summary, defaultOpen = false, persistKey, docsPath, children }: Props) {
  ...
  return (
    <div className="card collapsible-card">
      <div className="collapsible-header-row">
        ...
        {docsPath && <DocsHelpLink path={docsPath} label={title} />}
      </div>
