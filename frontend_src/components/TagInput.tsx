...
export function TagInput({
  ...
  const recoToShow = recommended.filter((tag) => !value.includes(tag) && canonicalTags.includes(tag)).slice(0, 8);
    ...
    <div className="tag-input-wrap">
      {showRecommended && recoToShow.length > 0 && (
        <div className="tag-reco">
          {recoToShow.map((tag) => {
            const { ns, qual } = splitTag(tag);
            return (
              <button type="button" key={tag} className="tag-reco-chip" onClick={() => add(tag)} title={tag}>
      ...
      <div className="tag-input" onClick={() => inputRef.current?.focus()}>
        {value.map((tag, i) => {
          ...
          return (
            <span key={tag} className="tag-chip" title={tag}>
              {ns && <span className="tag-chip-ns">{ns}</span>}
              <span className="tag-chip-qual">{qual}</span>
              <button type="button" className="tag-chip-x" aria-label={t("mesa.tagRemove", { tag })}
        ...
        {!atMax && (
          <input
            ...
            value={query}
            placeholder={value.length === 0 ? t("mesa.tagSearchPlaceholder") : ""}
            ...
            aria-label={t("mesa.semanticTags")}
        ...
        {open && matches.length > 0 && (
          <ul className="tag-suggest" role="listbox" id={listboxId}>
            {matches.map((tag, i) => {
              const { ns, qual } = splitTag(tag);
              return (
                <li key={tag} id={`${listboxId}-option-${i}`} role="option" aria-selected={i === active}
      ...
      {atMax && <div className="tag-input-note">{t("mesa.tagMax", { max })}</div>}
    </div>
  );
}
